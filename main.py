...
worker1 = Worker(name='ali abdi', skills={'cutting_cloth': 1., 'checking_quality': 0.5},
                 quality={'cutting_cloth': .8, 'checking_quality': 1.}, status='unemployed', tasks=[], valid=False)
# worker1 = Worker(name='ali abdi', skills={'checking_qua': 0.5},
#                  quality={'checking_qua': 1.}, status='unemployed', tasks=[], valid=False)

worker2 = Worker(name='mohamad molaei', skills={'sewing_sleeves': 0.9, 'sewing_colar': 0.7},
                 quality={'sewing_sleeves': 0.9, 'sewing_colar': 1.0}, status='unemployed', tasks=[], valid=False)

worker3 = Worker(name='iman najafi', skills={'sewing_colar': 1.}, quality={'sewing_colar': 1.}, status='unemployed',
                 tasks=[], valid=False)

worker4 = Worker(name='mina azizi', skills={'sewing_body': 1., 'connecting_parts': 0.6},
                 quality={'sewing_body': 1., 'connecting_parts': 1.}, status='unemployed', tasks=[], valid=False)

worker5 = Worker(name='azita hajian', skills={'connecting_parts': .9, 'sewing_body': 0.6, 'cutting_cloth': 1.0},
                 quality={'connecting_parts': 1., 'sewing_body': 0.9, 'cutting_cloth': 1.0}, status='unemployed',
                 tasks=[], valid=False)

worker6 = Worker(name='abas aghayi', skills={'printing_logo': 0.9},
                 quality={'printing_logo': 0.8}, status='unemployed', tasks=[], valid=False)

worker7 = Worker(name='rasool khadem', skills={'printing_logo': 1.},
                 quality={'printing_logo': 0.9}, status='unemployed', tasks=[], valid=False)

worker8 = Worker(name='hamed ghorbani', skills={'checking_quality': 1.},
                 quality={'checking_quality': 8.}, status='unemployed', tasks=[], valid=False)

worker9 = Worker(name='setare derakhshan', skills={'cutting_cloth': 0.8},
                 quality={'cutting_cloth': 1.0}, status='unemployed', tasks=[], valid=False)

worker10 = Worker(name='hamid bagheri', skills={'connecting_parts': 0.7, 'cutting_cloth': 0.5},
                  quality={'connecting_parts': 0.9, 'cutting_cloth': 1.0}, status='unemployed', tasks=[], valid=False)

worker11 = Worker(name='mahdi behboodi', skills={'connecting_parts': 1.0, 'cutting_cloth': 0.5, 'sewing_body': 0.7},
                  quality={'connecting_parts': 1.0, 'cutting_cloth': 1., 'sewing_body': 1.}, status='unemployed',
                  tasks=[], valid=False)

worker12 = Worker(name='amin ojaghi', skills={'connecting_parts': 0.6, 'printing_logo': 0.6},
                  quality={'connecting_parts': 0.9, 'printing_logo': 0.7}, status='unemployed', tasks=[], valid=False)

worker13 = Worker(name='rasool babayi', skills={'cutting_cloth': 0.6, 'printing_logo': 0.3},
                  quality={'cutting_cloth': 1., 'printing_logo': 0.9}, status='unemployed', tasks=[], valid=False)

worker14 = Worker(name='negar ahmadi', skills={'connecting_parts': 0.8, 'checking_quality': 0.9},
                  quality={'connecting_parts': 1., 'checking_quality': 1.}, status='unemployed', tasks=[], valid=False)

worker15 = Worker(name='mohamadreza ahmadi', skills={'sewing_colar': 0.3, 'sewing_body': 0.9},
                  quality={'sewing_colar': 1.0, 'sewing_body': 1.0}, status='unemployed', tasks=[], valid=False)

# define workers list
workers = [worker1, worker2, worker3, worker4, worker5, worker6, worker7, worker8, worker9, worker10, worker11,
           worker12, worker13, worker14, worker15]
# update their skills based on their qualities.
# workers = [worker.update() for worker in workers]
# # recreate worker list after update their skills
# workers = [worker1, worker2, worker3, worker4, worker5, worker6, worker7, worker8, worker9, worker10, worker11,
#            worker12, worker13, worker14, worker15]
...
product = Product('t-shert', tasks=[task1, task2, task3, task4, task5, task6, task7])

# assign tasks name for checking if there is any worker that his skills not in product tasks we keep him valid as False
# and at the end drop them from worker lists.
TASKS_NAME = []
for t in product.tasks:
    TASKS_NAME.append(t.name)

for index, worker in enumerate(workers):
    worker_skills = worker.skills.keys()
    for sk in worker_skills:
        if sk in TASKS_NAME:
            worker.valid = True

# just keep whose that their skills is in product tasks
valid_workers = [worker for worker in workers if worker.valid == True]

# Introduce a 5-second delay
print("finding the best hierarchy for this production...")
time.sleep(5)
...
make_plan(product=product, workers=valid_workers, stations=stations)
...
def assign_second_work(workers, tasks=product.tasks):
    for worker in workers:
        ...
        for skill in worker.skills:
            if skill in TASKS_NAME:
                another_skills_worker = [(skill, find_task(skill, tasks).take_time) for skill in worker.skills if
                                         skill != now_task[0].name]
                ...
                if free_time > 0:
                    if len(another_skills_worker) >= 1:
                        if free_time > another_skills_worker[0][1]:
                            if another_skills_worker[0][0] not in [task.name for task in worker.tasks]:
...
tasks = product.tasks
assign_second_work(workers=valid_workers, tasks=tasks)
# see the result.
...
# count how many workers we have (a) and how many of them is employed(c).
c = 0
a = 0
...
print(f"Number of workers that employed: {c}/{a}")

if a-c > 0:
    print(f"{a-c} workers has not employed beacause the skills which them have is not in product requirements.")
    for worker in unemployed_workers:
        print(worker.name, worker.skills)
